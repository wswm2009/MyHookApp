...
unsigned int __fastcall ldasm( void *code, ldasm_data *ld, unsigned char is64 )
/*
 Description:
 ...
 */
{
    ...
    unsigned char s,op,f;
    unsigned char rexw,pr_66,pr_67;
    ...
    ld->opcd_offset = (unsigned char)(p - (unsigned char*)code);
    ld->opcd_size    = 1;
    op = *p++; s++;
    ...
    if (f & OP_MODRM) {
        ...
        if (ld->disp_size) {
            ld->disp_offset = (unsigned char)(p - (unsigned char *)code);
            p += ld->disp_size;
            s += ld->disp_size;
            ld->flags |= F_DISP;
        }
    ...
    if (ld->imm_size) {
        s += ld->imm_size;
        ld->imm_offset = (unsigned char)(p - (unsigned char *)code);
        ld->flags |= F_IMM;
        if (f & OP_RELATIVE)
            ld->flags |= F_RELATIVE;
    }
...
unsigned long __fastcall SizeOfProc( void *Proc )
{
    ...
    do
    {
        Length = ldasm((unsigned char*)Proc, &data, is_x64);
...
void* __fastcall ResolveJmp( void *Proc )
{
    ...
    {
        unsigned char delta = *(unsigned char*)((size_t)Proc + data.opcd_size);
